...
app/                         # Website source (React 19 + Vite + Tailwind, TypeScript)
  ...
  de/ ru/ jp/ kr/ fr/        # German/Russian/Japanese/Korean/French mirror shells — GENERATED by
                             #   scripts/gen-shells.mjs from app/en/ + scripts/shell-i18n/, do not hand-edit
  src/
    i18n.tsx                 # zh/en dictionaries inline + LangProvider + useLang() — ALL site copy lives here
    lang/                    # de/ru/ja/ko/fr dictionaries (Record<TKey, ReactNode>, keys mirror dict.zh)
    download-data.ts         # VERSION + per-platform installer files & sizes (edit on every release)
    links.ts                 # MIRRORS table + cross-host helpers (mirrorOf / mirrorHref / rootHref)
    sections/                # Nav / LangMenu / Hero / Workbench / DayNight / Sanger / Toolbox / Download / Footer
    pages/                   # Home.tsx, TechSupport.tsx
  scripts/
    gen-shells.mjs           # generates app/{de,ru,jp,kr,fr}/*.html from app/en/ templates + hreflang upkeep
    shell-i18n/              # per-language shell head metadata (titles/descriptions/boot) for gen-shells.mjs
    prerender.mjs            # post-build headless render of all build pages (main site + 7 mirrors)
...
docs/                        # SERVED ROOT — do not hand-edit index.html / tech-support.html / assets/ / shots/
  ...
  de/ ru/ jp/ kr/ fr/        # BUILD OUTPUT (generated mirror shells, served at de/ru/jp/kr/fr.genepad.cn)
  ...
  release/                   # Installer downloads (windows/linux/android/mac) + langpacks/ (official
                             #   UI language packs de/fr/ja/ko/ru .json, linked from tutorial-langpack)
...
to `/<dir>/sitemap.xml`. Mirror discovery has three reinforcing channels:
per-mirror sitemaps, the hreflang alternates in every page's `<head>`, and the
footer language row (crawlable same-page links between all seven mirrors on
every page of every host).
...
The site is bilingual (zh/en) on the main host, with five additional
pure-language mirrors. All copy lives in `app/src/i18n.tsx` (zh + en inline);
the mirror-only languages de/ru/ja/ko/fr live in `app/src/lang/<code>.tsx`
(`Record<TKey, ReactNode>` whose key set must exactly mirror `dict.zh` —
`npm run build`'s `tsc` enforces this). Components call `t("key")` from
`useLang()`. On **genepad.cn** the language is chosen at runtime (localStorage
`genepad-lang`, falling back to the browser language, zh/en only) and the Nav
toggle swaps copy in place — this UX is deliberately unchanged. The exception:
each mirror host (and local dev `/en/` `/cn/` `/de/` `/ru/` `/jp/` `/kr/`
`/fr/` paths) locks its language via `links.ts mirrorOf()` and never reads
localStorage/navigator. When adding UI text, add `zh` + `en` entries in
i18n.tsx AND the same key in all five `src/lang/` dictionaries.

## Language Mirrors (en/cn/de/ru/jp/kr/fr .genepad.cn)

Seven crawler-facing pure-language mirrors of the bilingual pages (+ stats) —
`en.genepad.cn` (English), `cn.genepad.cn` (Chinese), `de.genepad.cn` (German),
`ru.genepad.cn` (Russian), `jp.genepad.cn` (Japanese, dir `jp`, lang `ja`),
`kr.genepad.cn` (Korean, dir `kr`, lang `ko`), `fr.genepad.cn` (French) — so
...
- **Shells**: 11 shells per mirror (index, tech-support, projects, library,
  ...
  `app/scripts/gen-shells.mjs` (runs inside `npm run build`, before vite):
  it takes the en shell as the structural template and rewrites the
  language bits (html lang, title/description/og/twitter/JSON-LD,
  canonical/hreflang, boot skeleton copy) from
  `app/scripts/shell-i18n/<code>.mjs`. The `pages.*.title` there must equal
  the `title.*` translations in `src/lang/<code>.tsx`. Never hand-edit the
  generated shells — edit the metadata and rerun.
- **Routing**: `docs/_worker.js` maps `<mirror>.genepad.cn/<path>` to the
  asset `/<dir><path>` (pure helper `route()`, unit-testable via node, mirror
  table `MIRROR_PREFIX_BY_HOST`). Shared prefixes (`/assets/` `/shots/`
  `/release/` `/api/` `/update.json` `/icon.*` `/robots.txt`) are never
  prefixed — note `/sitemap.xml` is NOT shared: mirror hosts serve their own
  generated `/<dir>/sitemap.xml`. On **any** host, `/<dir>/*` 308s to the
  mirror subdomain (covers `genepad.cn/de/*`, historical paths, cross-mirror
  links); `genepad.pages.dev` serves `/<dir>/*` statically as the pre-DNS
  preview.
- **Prerequisite (dashboard, one-time)**: Pages project → Custom domains →
  add each mirror subdomain (the zone's DNS record is created automatically).
  de/ru/jp/kr/fr were added 2026-09; until their deployment is confirmed,
  check the Pages check-run like any release push.
- **Language binding**: `i18n.tsx` `detectLang()` returns the mirror's
  language via `links.ts mirrorOf()` (hostname match first, then the
  `/<dir>/` path prefix for local dev / pages.dev / GitHub Pages); elsewhere
  the localStorage/browser detection runs as before (zh/en only).
- **Cross-host links**: `app/src/links.ts` — `MIRRORS` table (host + dir +
  language, single source shared conceptually with `_worker.js` /
  `prerender.mjs` / `gen-shells.mjs`), `mirrorHref(lang)` (same page on
  another mirror, absolute URL), `rootHref(name)` for zh-only pages
  (changelog, `tech-*.html`; must be absolute on mirror hosts, else the
  worker prefix-maps them into `/<dir>/` and 404s). `isMirrorHost()` = any
  mirror — Nav/SubpageNav use it to swap the in-place zh/en toggle for the
  `LangMenu` dropdown (lists all seven languages, jumps to the same page on
  the chosen mirror). `homeHref()` prefixes `/<dir>/` when statically hosted.
  The **Footer language row** (Footer.tsx, every page of every host) links all
  seven mirrors same-page with `hreflang` attributes — the main site's
  primary crawlable link into the mirrors; on a mirror the current language
  renders as plain text.
- **SEO pairing**: every bilingual page carries a full hreflang set (zh-CN →
  cn.genepad.cn, en → en.genepad.cn, de → de.genepad.cn, ru → ru.genepad.cn,
  ja → jp.genepad.cn, ko → kr.genepad.cn, fr → fr.genepad.cn, x-default →
  genepad.cn). gen-shells.mjs refreshes this block in ALL shells (including
  the hand-written en/cn/zh ones) on every build. Static `<title>`/og on zh
  pages are Chinese; en shells English; the five generated shells carry their
  own language's copy with their own host's canonical/og:url. `stats` carries
  no hreflang (noindex).
- **When adding/removing a bilingual page**: add/remove `app/<page>.html`,
  `app/en/<page>.html` AND `app/cn/<page>.html` (hand-written), list it in
  `gen-shells.mjs` PAGES + add its metadata to all five
  `scripts/shell-i18n/*.mjs`, and wire it into the mirror page lists in
  `vite.config.ts` and `prerender.mjs` (ROOT_PATHS; stats excluded from
  prerender). hreflang upkeep is automatic via gen-shells. Keep `rootHref()`
  for any zh-only links.
- The GitHub Pages mirror has no worker: `genepad.github.io/<dir>/...` serve
  the mirror shells directly — harmless, their canonicals point at the
  respective mirror subdomain.
- **UI-language tutorial + official language packs**: `tutorial-langpack`
  (教程五 · 设置界面语言, id "langpack" in `tutorial-data.ts`) is the
  entry point for the five official UI language packs, served from
  `docs/release/langpacks/GenePad-langpack-{de,fr,ja,ko,ru}.json` (version-agnostic
  filenames, compatible with app 0.7.x). The page renders a download table via
  `TutorialEntry.downloads` (root-relative `/release/langpacks/...` links —
  the shared-prefix worker rule keeps them working on every mirror host). Its
  discoverability path: Toolbox O-01 多语言 card "详细介绍" link on every home
  page (the main entry on the mirror sites), the tutorial-hub card, sidebar,
  and prev/next pagers — all derived automatically from `tutorial-data.ts`.
  To update a pack, overwrite the file (keep the name); no rebuild needed.
