...
export const PLATFORMS: (Omit<PlatformDownloads, "files"> & {
...
})[] = [
  {
    id: "windows",
    files: [
      { name: `GenePad_${VERSION}_Windows_amd64.zip`, size: "7.1 MB" },
    ].map(withSources),
  },
  {
    id: "mac",
    files: [
      { name: "macos-dmg.zip", size: "9.6 MB" },
      { name: "macos-app.zip", size: "8.5 MB" },
    ].map(withSources),
  },
  {
    id: "linux-x64",
    files: [
      { name: `GenePad_${VERSION}_Linux_amd64.deb`, size: "9.3 MB" },
      { name: `GenePad_${VERSION}_Linux_amd64.rpm`, size: "9.3 MB" },
      { name: `GenePad_${VERSION}_Linux_amd64.tar.gz`, size: "8.8 MB" },
    ].map(withSources),
    sparkStore: true,
  },
  {
    id: "linux-arm64",
    files: [
      { name: `GenePad_${VERSION}_Linux_arm64.deb`, size: "9.0 MB" },
      { name: `GenePad_${VERSION}_Linux_arm64.rpm`, size: "9.0 MB" },
      { name: `GenePad_${VERSION}_Linux_arm64.tar.gz`, size: "8.5 MB" },
    ].map(withSources),
    sparkStore: true,
  },
  {
    id: "android",
    files: [{ name: "app-universal-release.apk", size: "18.2 MB" }].map(withSources),
  },
];
