...
const ko: Record<TKey, ReactNode> = {
  ...
  "title.tutorial.ngs": "NGS 분석 튜토리얼 - GenePad | fastq.gz 조회 및 풍부도 리포트",
  ...
  "wb.1.name": "플라스미드 맵",
  "wb.1.desc":
    "원형 맵과 선형 맵을 한 번의 클릭으로 전환할 수 있습니다. 프로모터, CDS, 복제 시작점, poly(A) 신호 등의 요소가 카테고리별 색상으로 구분되고 레이블이 자동으로 겹침을 피해 배치되어, 요소가 많은 벡터에서도 맵을 선명하게 읽을 수 있습니다.",
  "wb.2.name": "맵 → 서열",
  "wb.2.desc":
  ...
  "wb.4.name": "단백질 속성",
  "wb.4.desc":
    "CDS를 선택하면 분자량, 등전점, GRAVY, 지방족 지수, 소광 계수가 실시간으로 계산됩니다. 등전점은 19가지 알고리즘이 내장되어 있어 서로 비교할 수 있으며, 기본적으로 문헌에서 권장하는 알고리즘의 결과를 표시합니다.",
  ...
  "tb.title": <>종합 도구 상자</>,
  "tb.lead":
    "서열을 선택하면 우클릭 메뉴에서 바로 겔 전기영동 시뮬레이션과 sgRNA 설계을 실행할 수 있습니다. 전역 도구 상자에는 AI 어시스턴트와 유전자 파일 라이브러리가 제공되며, 다국어 인터페이스와 'Open with' 지원으로 기존 워크플로와의 연동도 쉽습니다.",
  "tb.groupA": "컨텍스트 메뉴",
  "tb.groupB": "전역 도구 상자",
  "tb.groupC": "개방성",
  ...
  "tb.t2.desc":
    "연 적 있는 벡터 파일이 자동으로 색인되고, AI가 일괄로 특성 태그를 생성하며, 감시 폴더를 지원합니다. 파일 이름을 외울 필요 없이 플라스미드 속성으로 파일을 검색해 찾을 수 있고, AI 어시스턴트로도 검색할 수 있습니다. 색인만 생성할 뿐 원본 파일은 수정하지 않습니다.",
  "tb.o1.name": "다국어",
  "tb.o1.desc":
  ...
  "dl.upgradeNote":
    "GenePad가 이미 설치되어 있나요? 최신 설치 패키지를 내려받아 기존 버전 위에 설치하면 구버전을 제거할 필요 없이 업그레이드가 완료되며, 새 사용자는 내려받은 뒤 안내에 따라 설치하면 됩니다.",
  ...
  "dl.ai.desc":
    "macOS 설치 패키지는 서명되어 있지 않아 수동 설치에는 여러 단계가 필요합니다. 아래 프롬프트를 AI 어시스턴트(Kimi, ChatGPT 등)에 복사해 붙여 넣으면, 안내에 따라 단계별로 설치를 완료할 수 있습니다.",
...
⚠️ 주의: 위 방법은 동작하지 않거나 오래되었을 수 있습니다. 어느 단계를 완료할 수 없다면, 스스로 다른 실행 가능한 방법(예: Gatekeeper를 우회하는 다른 방식, 또는 다른 압축 해제·서명 도구 사용)으로 바꾸어 시도하세요. 위 다운로드 링크에 접근할 수 없을 때에만 공식 사이트 https://genepad.cn을 방문하세요. 공식 사이트는 다른 다운로드 링크만 제공하며 설치 지원은 제공하지 않습니다.`,
  ...
  "dl.mac.unsigned":
    "미서명 안내: 브라우저로 직접 내려받은 .dmg / .app 파일은 Gatekeeper가 '손상됨'으로 표시합니다. 우클릭 → '열기'로 실행하거나 xattr -cr을 실행해 격리 속성을 제거하세요. 위의 명령줄 설치 / 업그레이드 방식을 이용하는 것을 권장합니다.",
  "dl.copy": "복사",
  "dl.copied": "복사됨 ✓",
  ...
  "ts.gjson.desc":
    "JSON 교환 형식: 최상위 필드, 각 객체 정의, .gen과의 충실도 차이. 읽고 나면 .gjson을 파싱할 수 있습니다.",
  ...
  "pr.ca.desc":
    "독자적으로 구현한 통계 파이프라인: 공개된 GTEx 유전자 발현과 GENCODE 주석을 입력으로 사용하고 조직 발현량으로 가중치를 부여해, 인체 조직마다 코돈 사용 표를 재구축합니다. 코돈 최적화와 이종 발현 설계를 위한 재현 가능하고 검증 가능한 조직 특이적 참조를 제공하며, 라이선스 제한이 있는 서드파티 기성 참조표에 의존하지 않습니다.",
  ...
  "lib.pain.3.desc":
    "검색 의도는 보통 속성 묘사('특정 유전자를 운반하는 벡터')로 표현되지만, pLH-sgRNA1-Muc4-11 같은 파일 이름은 플라스미드의 특성을 반영하지 않아 둘 사이에 대응 관계가 없습니다.",
  ...
  "lib.ways.3.desc":
    "AI가 각 플라스미드의 서열과 주석을 읽어 lentiviral, sgRNA, ampicillin 같은 특성 태그를 자동 생성합니다. 태그를 클릭하면 관련 플라스미드를 모두 걸러 내며, 이름이 아닌 속성으로 찾을 수 있습니다.",

  "lib.tags.head": "AI 태그",
  "lib.tags.headEn": "AI TAGS",
  "lib.tags.title": "AI가 특성 태그를 자동 생성",
  "lib.tags.desc":
    "AI를 설정해 두면 플라스미드를 스캔하거나 열 때 프로그램이 자동으로 서열과 요소를 읽어 각 파일에 특성 태그 집합을 생성합니다. 표 헤더에서 일괄 새로 고침도 가능합니다. 누락된 태그만 채우거나 전체를 다시 생성할 수 있습니다. 오른쪽 태그 클라우드는 라이브러리 전체의 속성 분포를 보여 주며, 몇 가지 속성을 조합해 필터하면 대상 파일을 좁혀 찾을 수 있습니다.",
  ...
  "lib.ai.desc":
    "AI 어시스턴트에게 바로 지시할 수 있습니다. 예: 'Type I CRISPR 플라스미드 찾아 줘' — 어시스턴트가 라이브러리 전체를 검색해 후보와 각 플라스미드의 특성, 추천 용도를 설명해 줍니다. '내 플라스미드 라이브러리 정리해 줘'라고 요청하면 어시스턴트가 먼저 라이브러리 현황을 분석한 뒤 정리 방안을 제시합니다.",
  "lib.ai.shot1": "AI ASSISTANT — '내 플라스미드 라이브러리 정리': 먼저 현황 분석, 그다음 방안 제시",
  "lib.ai.shot2": "AI ASSISTANT — 'Type I CRISPR 플라스미드 찾기': 후보와 특성을 함께 나열",
  ...
  "lib.setup.3.desc":
    "브라우저가 DeepSeek 오픈 플랫폼을 엽니다. 계정을 등록하고 크레딧을 충전한 뒤 'Create API key'를 클릭합니다. 키는 한 번만 표시되므로 생성 즉시 복사해 두세요.",
  "lib.setup.3.shot": "DEEPSEEK 오픈 플랫폼 — API key를 만들고 즉시 복사",
  ...
  "np.lead":
    "fastq.gz / fastq / fq.gz / fq 시퀀싱 파일을 바로 엽니다. 우클릭이나 드래그 앤 드롭으로 열고, 페어 엔드 파일은 자동으로 매칭·병합됩니다. 염기별 시퀀싱 품질 확인, 아미노산 단편 검색으로 가변 영역 위치 찾기, 앵커를 설정한 일괄 트리밍, 원클릭 라이브러리 풍부도 리포트 생성까지 지원합니다.",
  "np.c1": "fastq.gz · fq.gz 압축 해제 불필요",
  "np.c2": "페어 엔드 자동 매칭·병합",
  "np.c3": "염기별 시퀀싱 품질",
  "np.c4": "앵커 트리밍 + 풍부도 리포트",
  "np.cta": "자세히 보기",
  ...
  "ngs.lead":
    "GenePad는 fastq.gz / fastq / fq.gz / fq 시퀀싱 파일을 바로 엽니다. 파일 탐색기에서 우클릭 'Open with'로 GenePad를 선택하거나 파일을 프로그램 창으로 끌어다 놓으세요. 페어 엔드 시퀀싱은 R1, R2 두 파일을 함께 끌어다 놓으면 페어 관계가 자동 인식됩니다. 연 뒤에는 리드를 한 줄씩 열람하고 염기별 시퀀싱 품질과 페어 병합 결과를 확인할 수 있으며, 아미노산 단편으로 검색해 목표 가변 영역을 찾고, 트리밍 앵커를 설정해 파일 전체를 일괄 트리밍한 다음, 원클릭으로 라이브러리 풍부도 리포트를 생성할 수 있습니다.",
  "ngs.hero.shot":
    "FASTQ VIEWER — 페어 리드 자동 병합, 염기별 품질 색상, 오른쪽은 파일 통계",
  ...
  "ngs.reads.title": "모든 리드의 품질과 병합 결과를 한눈에",
  "ngs.reads.lead":
    "메인 화면에는 시퀀싱된 모든 리드가 위에서 아래로 나열되고, 오른쪽 속성 패널에 파일 전체의 시퀀싱 통계가 정리됩니다:",
  "ngs.reads.1.name": "페어 엔드 자동 병합",
  "ngs.reads.1.desc":
    "페어 엔드 데이터는 한 줄씩 자동 병합(merge)되며, 각 리드에 겹침 길이와 일치도가 표시됩니다(예: Overlap 135bp · 99% identity). 병합 결과를 한눈에 판별할 수 있습니다.",
  ...
  "ngs.reads.shot":
    "READS & QUALITY — 위에서 아래로 한 리드씩: 페어 병합, 염기별 품질 막대, 오른쪽 파일 통계",

  "ngs.aa.head": "아미노산 검색",
  "ngs.aa.headEn": "SEARCH BY AMINO ACIDS",
  "ngs.aa.title": "단백질 단편으로 관심 가변 영역 찾기",
  "ngs.aa.desc":
    "아래쪽 검색 상자를 'AA' 모드로 바꾸고 아미노산 서열(예: MATNNQ)을 입력하면, 프로그램이 병합된 리드를 단백질로 번역해 한 줄씩 비교하고 매칭된 펩타이드를 서열 속에서 바로 표시합니다. 라이브러리 시퀀싱은 양쪽 서열이 보존되고 가운데가 가변적입니다. 알려진 보존 단백질 단편을 질의로 쓰면 수천 개 리드에서 각자의 목표 가변 영역을 빠르게 찾을 수 있습니다.",
  "ngs.aa.shot": "AA SEARCH — 아미노산 단편 입력, 매칭된 펩타이드가 번역 결과에서 표시됨",

  "ngs.trim.head": "앵커 트리밍",
  "ngs.trim.headEn": "TRIM ANCHORS",
  "ngs.trim.title": "트리밍 앵커를 설정해 목표 구간을 정밀하게 잘라 내기",
  "ngs.trim.desc":
    "목표 구간 양쪽의 보존 서열을 좌우 트리밍 앵커(trim anchors)로 지정하고 'Trim'을 클릭하면 시퀀싱 파일 전체가 일괄 트리밍되어 두 앵커 사이의 가변 영역 서열만 남습니다. 속성 패널에는 통과 비율이 실시간 표시되고, 앵커 미적중이나 단편이 너무 짧은 등으로 제외된 수량도 알려 줍니다. 절단된 리드는 원클릭으로 내보낼 수 있어('Export processed reads') 후속 분석이나 제출에 쓸 수 있습니다.",
  "ngs.trim.shot": "TRIM ANCHORS — 좌우 앵커가 목표 창을 표시, 통과 비율 실시간 표시",

  "ngs.report.head": "라이브러리 풍부도 분석",
  "ngs.report.headEn": "LIBRARY ANALYSIS",
  "ngs.report.title": "파일 전체를 한 번의 클릭으로 풍부도 리포트로 완성",
  "ngs.report.desc":
    "'Sequence analysis'를 클릭하면 절단 결과에 대한 통계를 실행합니다. 기본적으로 파일 전체를 분석하며(수천만 리드를 메모리에 모두 올릴 필요 없음), 고유한 서열마다 출현 횟수와 빈도를 집계하고 양쪽 앵커를 유지하거나 제거할지 선택할 수 있습니다. 분석이 끝나면 그림과 표로 이루어진 리포트가 생성됩니다. Top 서열 히스토그램(DNA / AA 두 기준)과 서열별 길이, 카운트, 빈도, +1 리딩 프레임 번역이 담기고, 전체 표는 CSV로 함께 내보내져 Excel에서 바로 열 수 있습니다. 간단한 라이브러리 풍부도 리포트가 이것으로 완성됩니다.",
  "ngs.report.shot1": "SEQUENCE ANALYSIS — 기본은 파일 전체 분석, 앵커 유지 / 제거 선택 가능",
  "ngs.report.shot2": "ANALYSIS REPORT — Top 서열 히스토그램과 전체 풍부도 표 (CSV)",

  "ngs.video.head": "비디오 튜토리얼",
  "ngs.video.headEn": "VIDEO TUTORIAL",
  "ngs.video.lead":
    "글과 그림만으로 부족하신가요? 전체 조작 과정을 담은 데모가 있습니다. 시퀀싱 파일 열기, 페어 엔드 페어링부터 앵커 트리밍, 풍부도 리포트 생성까지 실제 데이터로 전 과정을 단계별로 진행하며 페이지 안에서 바로 재생할 수 있습니다.",
  "ngs.video.caption": "VIDEO TUTORIAL — 전체 과정 데모: 열기 → 페어링 → 열람 → 검색 → 절단 → 풍부도 리포트",
  ...
  "tut.lead":
    "GenePad의 그림 튜토리얼을 한자리에 모았으며 앞으로도 새 편을 계속 추가할 예정입니다. DeepSeek를 연결해 AI 기능을 열고, 유전자 파일 라이브러리로 수백 개 플라스미드를 관리하고, fastq.gz부터 라이브러리 풍부도 리포트까지 NGS 분석을 완주하는 것까지. 모든 단계에 실제 화면 스크린샷이 붙어 있으니 순서대로 따라 하면 됩니다.",
  ...
  "tut.toc.lib.desc":
    "감시 폴더의 플라스미드가 자동으로 라이브러리에 수집되고, 프로젝트·경로·태그 세 가지 방식으로 검색하며, AI 어시스턴트가 속성으로 플라스미드를 찾아 줍니다.",
  "tut.toc.lib.en": "FILE LIBRARY",
  "tut.toc.ngs.name": "NGS 파일 분석",
  "tut.toc.ngs.desc":
    "fastq.gz를 끌어다 놓으면 시작합니다. 페어 엔드 페어링, 염기별 품질 검사, 아미노산 검색, 앵커 트리밍, 풍부도 리포트.",
  ...
  "tut.ai.3.desc":
    "프로파일 이름을 정하고, 공급자는 DeepSeek, 모델은 deepseek-v4-flash를 선택하며, API URL은 기본값 그대로 둡니다. 그다음 API Key 옆의 'Get'을 클릭하면 브라우저가 DeepSeek 오픈 플랫폼을 엽니다.",
  "tut.ai.3.shot": "이름과 모델 입력 — API Key 옆의 'Get' 클릭",
  "tut.ai.4.name": "DeepSeek API 키 발급",
  "tut.ai.4.desc":
    "DeepSeek 오픈 플랫폼에서 계정을 등록하고 크레딧을 충전한 뒤 'Create API key'를 클릭하고 이름을 붙여 확인합니다. 키는 한 번만 표시되니 즉시 복사하세요.",
  ...
  "tut.lib.2.desc":
    "새 프로젝트를 만들고 플라스미드를 끌어다 놓으면 분류가 끝납니다. 저장 경로 그대로 탐색할 수도 있습니다. AI를 설정하면 플라스미드마다 특성 태그가 붙어 태그 클릭만으로 관련 플라스미드를 모두 걸러 냅니다.",
  "tut.lib.3.name": "AI가 특성 태그 자동 생성",
  "tut.lib.3.desc":
  ...
  "tut.lib.4.desc":
    "AI 어시스턴트에게 바로 요구를 말하면 됩니다. 예: 'Type I CRISPR 플라스미드 찾아 줘' — 어시스턴트가 라이브러리 전체를 검색해 후보와 각 플라스미드의 특성, 추천 용도를 설명해 줍니다.",
  ...
  "tut.ngs.3.desc":
    "메인 화면에 리드가 한 줄씩 나열됩니다. 염기는 Phred 품질에 따라 색칠되고, 페어 엔드 데이터는 자동 병합되며 겹침 길이와 일치도가 표시됩니다. 오른쪽 속성 패널에는 리드 수, GC 함량 등 파일 통계가 정리됩니다.",
  "tut.ngs.4.name": "아미노산 검색으로 가변 영역 찾기",
  "tut.ngs.4.desc":
    "아래쪽 검색 상자를 'AA' 모드로 바꾸고 알려진 보존 단백질 단편(예: MATNNQ)을 입력합니다. 프로그램이 병합된 리드를 단백질로 번역해 한 줄씩 비교하고 매칭된 펩타이드를 바로 표시하며, 수천 개 리드에서 목표 가변 영역을 빠르게 찾습니다.",
  "tut.ngs.5.name": "앵커 설정 후 일괄 트리밍",
  "tut.ngs.5.desc":
    "목표 구간 양쪽의 보존 서열을 좌우 트리밍 앵커로 지정하고 'Trim'을 클릭하면 파일 전체가 일괄 트리밍되어 두 앵커 사이의 가변 영역만 남습니다. 속성 패널에 통과 비율이 실시간 표시되며 절단 결과는 원클릭으로 내보낼 수 있습니다.",
  "tut.ngs.6.name": "라이브러리 풍부도 리포트 생성",
  "tut.ngs.6.desc":
  ...
  "st.os.other": "기타",
  "st.updated": "데이터 업데이트",
  "st.note": "집계 방식: 무작위 설치 식별자로 집계합니다. 통계를 끄거나 앱을 제거하면 더 이상 계수하지 않습니다.",
