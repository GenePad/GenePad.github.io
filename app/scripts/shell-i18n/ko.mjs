...
export default {
  ...
  pages: {
    ...
    "ngs": {
      title: "NGS 뷰어 - GenePad | FASTQ 데이터 조회 및 라이브러리 풍도 분석",
      description:
        "GenePad 내장 NGS 데이터 뷰어: 우클릭 'Open With' 또는 드래그 앤 드롭으로 fastq.gz / fastq / fq.gz / fq 파일을 바로 엽니다. 페어 엔드 파일 자동 매칭·조립, 염기별 시퀀싱 품질, 아미노산 단편 검색으로 가변 영역 위치 찾기, 앵커 절단 일괄 캡처, 원클릭 라이브러리 풍도 리포트.",
    },
    "tutorial": {
      title: "튜토리얼 - GenePad | AI 설정 · 파일 라이브러리 · NGS 분석",
      description:
        "GenePad 튜토리얼 목록: 그림으로 단계별 안내하는 가이드 — DeepSeek AI 설정, 유전자 파일 라이브러리 정리, NGS 데이터를 fastq.gz부터 라이브러리 풍도 리포트까지. 한 편당 한 페이지로 계속 추가됩니다.",
    },
    "tutorial-ai": {
    ...
    "tutorial-ngs": {
      title: "NGS 분석 튜토리얼 - GenePad | fastq.gz 조회 및 풍도 리포트",
      description:
        "GenePad 단계별 튜토리얼: 드래그 앤 드롭으로 fastq.gz 열기, 페어 엔드 자동 페어링, 염기별 품질 확인, 아미노산 검색, 앵커 절단, 라이브러리 풍도 리포트 생성 — 전체 과정 비디오 포함. 모든 단계를 실제 스크린샷으로 그림 해설합니다.",
    },
    "tutorial-lang": {
